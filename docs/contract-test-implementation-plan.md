...
test('P-03: should reject product with invalid price_cents', async ({ request }) => {
  const response = await request.post('/api/v1/products', {
    data: {
      title: 'Test Product',
...
test('P-04: should reject product with invalid condition', async ({ request }) => {
  const response = await request.post('/api/v1/products', {
    data: {
      title: 'Test Product',
      price_cents: 1500000,
...
test('P-07: should filter products by category', async ({ request }) => {
  const response = await request.get(`/api/v1/products?category_id=${categoryId}`);
...
test('P-09: should return 404 for non-existent product', async ({ request }) => {
  const response = await request.get('/api/v1/products/00000000-0000-0000-0000-000000000000');
...
test('P-12: should approve submitted product', async ({ request }) => {
  ...
  const approveResp = await request.post(`/api/v1/products/${product.id}/approve`);
...
test('P-13: should reject product', async ({ request }) => {
  ...
  await request.post(`/api/v1/products/${product.id}/submit`);
  const rejectResp = await request.post(`/api/v1/products/${product.id}/reject`, {
