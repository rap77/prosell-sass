...
Implement `update_listing_task.py` — same DI pattern as `delete_listing_task.py` but calls `service.update()` instead of `service.delete()`:
```python
...
async def update_listing_task(publication_id: str) -> dict:
    ...
    - Marks publication updated_at on success
    ...
    async with async_session_factory() as session:
        ...
        try:
            # image_bytes_list=[] — images are already stored in DO Spaces and referenced
            # via publication.image_urls. The publisher service re-downloads from those URLs.
            ...
            return {"status": "updated", "fb_listing_id": publication.fb_listing_id, "engine": engine_name}
