# Code Generation Questions — u1-cross-org-export-api

## Plan Approval

Resumen del plan (`code-generation-plan.md`, 13 steps, metodología
test-after):

- **Repositorio**: `AbstractOrganizationRepository.get_by_ids()` nuevo
  (batch, BR2.3) + tests de integración.
- **Dominio**: `category_translation.py` nuevo (dict estático,
  vertical→`category`/`type` cliente) + extensión de
  `build_client_format_row()`/`csv_export.py` para corregir el mapeo de
  8 columnas (FR7, BR1.1-1.8: `vin`, `body_type`→`body_style`,
  `title_status`→`clean_title` inverso, `facebook_groups`→`groups` con
  fallback, `title_state`→`state`, `location_city`+`location_state`→
  `location`, `path` nuevo) + tests unitarios (piso mínimo puntos 1 y 3).
- **Caso de uso**: `ExportCatalogClientFormatUseCase.execute()`
  extendido con `all_organizations`, `base_folder`,
  `facebook_groups_fallback`, `organization_id` opcional; resolución
  batch de `org_code`; cache lazy de vertical de categoría; exclusión de
  productos sin traducción (BR1.7); cap global reusando
  `count(tenant_id=None)` ya soportado por el repositorio existente +
  tests (piso mínimo puntos 2 y 3).
- **Endpoint**: `product_router.py` extendido con los 3 query params
  nuevos + extensión de `_check_org_scope_permission()` +
  `catalogo_TODAS_{fecha}.zip` + tests de integración (piso mínimo
  punto 2, defensa en profundidad).
- Sin migración de base de datos, sin dependencia nueva.
...
[Approval Fingerprint]: sha256:9310f82c40f981e0656cbfb3eeb3d6dd44403456fcb5be8c0da9658e3b2f2daf
