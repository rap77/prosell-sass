# Team Practices — 260915-vehicle-catalog

> Integración final (Step 5 de practices-discovery), tras revisión ciega de
> quality/developer/devsecops y entrevista humana (Step 4). Este es un RE-RUN:
> `memory/team.md` ya tiene contenido afirmado extenso de intents previos.
> Cada sección parte de ese baseline — se referencia, no se repite verbatim —
> y marca explícitamente qué es **reconfirmación** vs. qué sería
> **especialización nueva propuesta** para este intent puntual (catálogo
> canónico de vehículos de Facebook: schemas de categoría, decode de VIN,
> import/validación, defaults de ubicación por-producto, migración legacy,
> contratos de adapter de publisher).
...
Las tres revisiones ciegas (quality, developer, devsecops) coincidieron en
AGREE de fondo sobre que el baseline general de las 5 secciones (Way of
Working, Walking Skeleton, Deployment, Code Style, y metodología/ordering de
Testing Posture) no necesita especialización nueva de proceso de equipo —
solo señalaron correcciones objetivas y gaps puntuales al piso de test y a
la sección de seguridad, ya integrados arriba en § Testing Posture, §
Deployment y § Code Style. La entrevista humana (Q1, Q2) afirmó el piso de 6
puntos tal cual (incluyendo los 2 gaps de QA) y la sanitización de fórmulas
en el export CSV. Ninguna promoción a `discovered-rules.md` § Mandated —
mismo criterio ya aplicado en intents previos: decisiones de diseño acotadas
a este intent puntual van acá y en `evidence.md`, no a restricciones de
proceso de equipo general.
