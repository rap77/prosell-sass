# Code Generation Plan — 260901-frontend-test-debt

Zero-Unit directive (bugfix scope, no Units Generation). Single implementation
iteration covering all of `requirements.md` (FR1-FR4, NFR1).
...
**Adaptation note**: only the "Frontend behavior" testable layer applies here
(Data model, Repository, Business logic, API/endpoint are all inapplicable —
FR4.1: zero production-code change). This intent's targeted regression IS the
existing failing tests themselves (FR1.2/FR2.2/FR3.2) — no new test files are
written (requirements.md FR4.3/C2, "minimal patch" answer to Q3). "Implement"
below means patching the mock fixture; "write and run tests" collapses to
"run the existing tests" per test-after ordering, since the tests already
exist and only their fixtures are stale.

## Plan Steps

- [ ] **Step 1**: Verify the existing test runner/configuration and record the
      exact unit-scoped commands (traceability → FR1-FR3 verification).
  - Command per file: `pnpm vitest run <relative-path>` from `apps/web/`.
- [ ] **Step 2**: Frontend behavior — implement: patch `products.test.tsx`
      mocks (FR1.1). Story: "products.test.tsx tests pass again" (FR1).
- [ ] **Step 3**: Frontend behavior — run `products.test.tsx`'s tests after
      the patch; confirm all 12 pass (FR1.2).
- [ ] **Step 4**: Frontend behavior — implement: patch
      `reverseTransitions.test.tsx`'s shared `mockProductResponse()` helper
      (FR2.1). Story: "reverseTransitions.test.tsx tests pass again" (FR2).
- [ ] **Step 5**: Frontend behavior — run `reverseTransitions.test.tsx`'s
      tests after the patch; confirm all 9 pass (FR2.2).
- [ ] **Step 6**: Frontend behavior — implement: patch
      `setProductCover.test.ts`'s `productFixture()` helper (FR3.1). Story:
      "setProductCover.test.ts tests pass again" (FR3).
- [ ] **Step 7**: Frontend behavior — run `setProductCover.test.ts`'s tests
      after the patch; confirm all 3 pass (FR3.2).
- [ ] **Step 8**: Verify no other file was touched (FR4.1/FR4.3) — `git diff
--stat` shows exactly the 3 test files.
- [ ] **Step 9**: Run vitest/tsc/eslint on the 3 touched files (project's
      already-learned Code Generation convention: verify touched files in this
      stage, not only in Build and Test).
- [ ] **Step 10**: Documentation and traceability — write `code-summary.md`
      and `traceability.json`.

No steps for: Data model/database, Repository/data access, Business logic,
API/endpoint, Environment/build configuration — none apply (FR4.1: zero
