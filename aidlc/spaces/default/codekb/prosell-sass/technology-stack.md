...
| Validación de esquema         | Zod                                            | `^4.4.0` instalado desde 2026-07-20 — **código en estilo Zod 3** (36 call sites `.passthrough()` en 14 archivos, 4 `z.nativeEnum()` en 2 archivos); migración de sintaxis pendiente (issue #74 relacionado ya está CERRADO, alcance distinto), ver `code-quality-assessment.md` |
...
## Confirmación de vigencia — scan enfocado `260828-zod-3-to-4-migration`

El scan enfocado de este intent (foco `AGENTS.md`, `apps/web/package.json`, `apps/web/src/lib/api/schemas/`, `apps/web/src/lib/api/extractErrorMessage.ts`/`verticals.ts`, `apps/web/src/components/forms/UnifiedProductForm.tsx`/`MemberForm.tsx`) confirma directamente, sin cambios de versión respecto al pase anterior: `zod: ^4.4.0` (instalado desde el commit `d1af1858`/`6042236b`, 2026-07-20 — no es una instalación nueva de este intent) y `@hookform/resolvers: ^5.4.0` (bumped en el mismo commit). No introduce ninguna dependencia nueva — es limpieza de sintaxis sobre un paquete ya instalado. Confirma además, vía `gh issue view 74`, que el issue GitHub que motiva la nota "Zod 3 hasta resolver #74" en `AGENTS.md` está **CERRADO** (2026-07-20) y que su alcance propio (mensajes de error `.min(n, "msg")` → `{ error: ... }`) es distinto e independiente del alcance de este intent (`.passthrough()` → `z.looseObject()`, `z.nativeEnum()` → `z.enum()`).

## Confirmación de vigencia — scan enfocado `260903-catalog-client-export`
...
## Confirmación de vigencia — scan enfocado `260911-cross-org-export-ux`

El scan enfocado de este intent (foco: export "todas las organizaciones", filtrado real del catálogo por organización, formato CSV cliente correcto, popups de `path`/`groups`: `OrganizationPicker.tsx`, `organizationStore.ts`, `organizations.ts`, `schemas/organizations.ts`, `catalog/page.tsx`, `products.ts`, `csv_export.py`, `export_catalog_client_format.py`, `product_router.py`, `admin_organizations_router.py`, `product.py`, `product_repository_impl.py`, `category_repository_impl.py`, `csv_field_mapper.py`, `bulk_upload_vehicles.py`, `seed_categories.py`, `docs/data39.csv`, `product_model.py`, `category_model.py`) no encontró cambios de versión respecto al pase anterior, y confirma que **no se requiere ninguna dependencia nueva**: es un conjunto de fixes de wiring frontend (filtrado de picker/grilla usando datos ya expuestos por el backend), de orquestación backend (exponer una capacidad de repositorio ya existente a través del use case de export) y de mapeo de datos (corregir/completar transformaciones de valor entre `attributes` y las columnas del CSV cliente, reutilizando lógica de mapeo inverso ya implementada para el import). Zustand `^5.0.11`, TanStack Query `^5.0.0`, Next.js `^16.3.3`, React `^19.2.8`, FastAPI `[standard]==0.128.0`, Pydantic `2.12.5` — sin cambios.

## Confirmación de vigencia — scan enfocado `260915-vehicle-catalog`

El scan enfocado de este intent (foco: catálogo editorial de valores Facebook vs. normalización de VIN decode, categorización de vehículos, publisher: `category_repository_impl.py`, `seed_categories.py`, `category.py`, `category_translation.py`, `category-schema-editor.tsx`, `bulk_upload_vehicles.py`, `csv_field_mapper.py`, `product.py`, `vehicle_router.py`, `nhtsa_normalizer.py`, `i_publisher_service.py`, `publisher_strategy.py`, `VinDecodeField.tsx`, dos migraciones Alembic, `export_catalog_client_format.py`) no encontró cambios de versión respecto al pase anterior, y confirma que **no se requiere ninguna dependencia nueva**: `@dnd-kit` (drag-and-drop del schema editor) ya está instalado y en uso desde antes de este intent; la desalineación de catálogos de valores es un problema de contrato interno entre dos módulos ya existentes, no de librería faltante. `nhtsa_normalizer.py` consume la NHTSA VPIC API externa (sin cliente HTTP dedicado nuevo); `facebook-values/index.ts` es un catálogo estático en TypeScript, sin dependencia de red. Next.js `^16.3.3`, React `^19.2.8`, FastAPI `[standard]==0.128.0`, Pydantic `2.12.5`, SQLAlchemy 2.0 async — sin cambios.
