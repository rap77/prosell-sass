...
- **Gap de permiso cross-org confirmado (scan enfocado `260910-export-cross-org`)**: `export-client-format.zip` resuelve `tenant_id` exclusivamente de `current_user.tenant_id` — no implementa ninguno de los 3 patrones de acceso cross-org que sí coexisten en el resto del router (`_check_org_scope_permission()`, `is_org_admin` inline, `has_role("super_admin")`). Un `super_admin`/`ORG_ADMIN_VIEW_ALL` no puede hoy exportar el catálogo de otra organización, pese a que sí puede verla/gestionarla en el resto de la app. Confirmado por los propios artefactos de diseño del intent `260903-catalog-client-export` (`personas.md`) como decisión de scope explícita, no un bug de implementación. Ver `architecture.md` § Interaction Diagrams (diagrama 13) y § "Modelo de permisos cross-org de `product_router.py`" en `api-documentation.md`.
