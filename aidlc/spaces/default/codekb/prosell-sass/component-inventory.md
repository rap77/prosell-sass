...
- **Gap de wiring de UI — vigente (scan enfocado `260911-export-org-selector`)**: pese a que el backend ya acepta `organization_id`, `catalog/page.tsx`/`exportCatalogClientFormat()` nunca lo mandan — la app ya tiene un selector cross-org de UI reusable (`OrganizationPicker`, ver arriba) pero desconectado de este flujo. Bifurcación de diseño pendiente (cablear al `viewingOrgId` global vs. selector local propio) — ver `architecture.md` § Interaction Diagrams (diagrama 14).
...
- **Estado**: componente maduro y en producción, pero su estado (`viewingOrgId`) está **dormido** — censo completo de `viewingOrgId` en `apps/web/src` confirma que ningún hook de datos de la app (catálogo, review-queue, ni otro) lo consume para filtrar nada hoy; solo el propio picker lo lee/escribe. Candidato natural (opción a) a conectar al flujo de export de catálogo, en bifurcación abierta contra un selector local independiente (opción b) — ver `architecture.md` § Interaction Diagrams diagrama 14 y `code-quality-assessment.md` para el detalle completo, sin resolver en este scan.
