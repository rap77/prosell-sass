...
describe("extractErrorMessage", () => {
  it("extracts a string detail", () => {
    expect(extractErrorMessage({ detail: "owner_email is already registered" }, "fallback")).toBe(
      "owner_email is already registered",
    );
  });
  ...
  it("extracts the first msg from a FastAPI 422 validation error array", () => {
    ...
    expect(extractErrorMessage(body, "fallback")).toBe("value is not a valid email address");
  });
