  ...
  test("MVP-001: Complete Admin Flow", async ({ page }) => {
    ...
    await test.step("Verify dashboard elements", async () => {
      ...
      const cards = page.locator(
        '[data-testid="dashboard-card"], .card, .widget',
      );
    ...
    await test.step("Access vehicles catalog", async () => {
      ...
      if (!is404) {
        ...
        const vehicleList = page.locator(
          '[data-testid="vehicle-list"], .vehicle-list',
        );
    ...
    await test.step("Create test vehicle", async () => {
      ...
      if (formExists) {
        ...
        const vinField = page
          .locator(
            'input[name="vin"], input[placeholder*="VIN" i], input[id*="vin" i]',
          )
          .first();
        ...
        const makeField = page
          .locator('input[name="make"], input[name="brand"]')
          .first();
        ...
        if ((await submitButton.count()) > 0) {
          ...
          const successMessage = page.locator(
            "text=/success|created|vehicle created/i",
          );
          ...
          if ((await successMessage.count()) > 0) {
            ...
            console.log(
              "⚠️  Vehicle form has validation errors (expected for minimal data)",
            );
          } else {
