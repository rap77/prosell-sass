...
function PwField({
  id, label, value, onChange, onBlur, hasError, errorMsg, disabled,
}: {
  id: string; label: string; value: string
  onChange: (v: string) => void; onBlur: () => void
  hasError: boolean; errorMsg?: string; disabled: boolean
}) {
  const [show, setShow] = useState(false)
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label htmlFor={id} style={{ fontSize: 13, fontWeight: 500, color: 'var(--ps-text-secondary)' }}>
        {label}
      </label>
      <div style={{ position: 'relative' }}>
        <input
          ...
          onBlur={(e)  => { blurAuthInput(e.currentTarget, hasError); onBlur() }}
...
function OAuthBtn({ label, icon, loading, onClick }: { label: string; icon: React.ReactNode; loading: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} disabled={loading}
      style={{ height: 40, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'transparent', border: '1px solid var(--ps-input-border)', borderRadius: 8, color: 'var(--ps-text-secondary)', fontSize: 13, fontWeight: 500, cursor: loading ? 'not-allowed' : 'pointer', transition: 'border-color 180ms, color 180ms, background 180ms', opacity: loading ? 0.6 : 1 }}
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--ps-border-strong)'; e.currentTarget.style.color = 'var(--ps-text-primary)'; e.currentTarget.style.background = 'var(--ps-hover-bg-xs)' }}
      onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--ps-input-border)'; e.currentTarget.style.color = 'var(--ps-text-secondary)'; e.currentTarget.style.background = 'transparent' }}
    >
      {icon}{label}
    </button>
  )
...
export function RegisterPageContent() {
  ...
  return (
    <AuthShell>
      <AuthFormHead title="Creá tu cuenta" subtitle="14 días gratis · sin tarjeta de crédito" />
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* Full name */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label htmlFor="fullName" style={{ fontSize: 13, fontWeight: 500, color: 'var(--ps-text-secondary)' }}>
            Nombre completo
          </label>
          <input
          ...
          />
          {errors.fullName && <AuthFieldError message={errors.fullName.message ?? ''} />}
        </div>

        {/* Email */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label htmlFor="reg-email" style={{ fontSize: 13, fontWeight: 500, color: 'var(--ps-text-secondary)' }}>
            Email
          </label>
          <input
        ...
        <Controller name="password" control={control} render={({ field, fieldState }) => (
          <PwField id="reg-password" label="Contraseña" value={field.value} onChange={field.onChange} onBlur={field.onBlur} hasError={!!fieldState.error} errorMsg={fieldState.error?.message} disabled={isDisabled} />
        )} />

        {/* Confirm password */}
        <Controller name="confirmPassword" control={control} render={({ field, fieldState }) => (
          <PwField id="reg-confirm-password" label="Confirmar contraseña" value={field.value} onChange={field.onChange} onBlur={field.onBlur} hasError={!!fieldState.error} errorMsg={fieldState.error?.message} disabled={isDisabled} />
        )} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
              <input
                ...
                disabled={isDisabled}
                style={{ marginTop: 2, accentColor: 'var(--ps-cyan)', width: 16, height: 16, flexShrink: 0 }}
              />
              <span style={{ fontSize: 13, color: 'var(--ps-text-secondary)', lineHeight: 1.5 }}>
                Acepto los{' '}
                <Link href="/terms" style={{ color: 'var(--ps-cyan)', textDecoration: 'none', fontWeight: 500 }}>Términos de Servicio</Link>
                {' '}y la{' '}
                <Link href="/privacy" style={{ color: 'var(--ps-cyan)', textDecoration: 'none', fontWeight: 500 }}>Política de Privacidad</Link>
              </span>
            </label>
        ...
        <AuthSubmitButton label="Crear cuenta" loadingLabel="Creando cuenta..." isLoading={isLoading} disabled={isDisabled} />
      </form>

      <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--ps-text-secondary)', margin: 0 }}>
        ¿Ya tenés cuenta?
        <Link href="/auth/login" style={{ color: 'var(--ps-cyan)', textDecoration: 'none', fontWeight: 600, marginLeft: 4 }}>
          Iniciar sesión →
        </Link>
      </p>
    </AuthShell>
