    ...
    <AuthShell>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, textAlign: 'center' }}>
        ...
        <div style={{
          ...
          background: isSuccess ? 'var(--ps-success-bg)' : 'var(--ps-error-bg)',
          border: `1px solid ${isSuccess ? 'rgba(34,211,160,0.25)' : 'rgba(240,68,56,0.25)'}`,
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
        }}>
          {isSuccess
            ? <CheckCircle2 size={28} style={{ color: 'var(--ps-success)' }} strokeWidth={1.8} />
            : <AlertTriangle size={28} style={{ color: 'var(--ps-error)' }} strokeWidth={1.8} />
          }
        </div>
        ...
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--ps-text-secondary)', margin: 0, lineHeight: 1.6, maxWidth: 320 }}>
            {isSuccess
...
export function ResetPasswordPageContent({ token }: ResetPasswordPageContentProps) {
  ...
  const onSubmit = async (data: ResetValues) => {
    if (!token || token.trim() === '') { setTokenInvalid(true); return }
    setApiError(null)
    try {
      await authApi.resetPassword(token, data.password)
      setSuccess(true)
    } catch (err) {
      setApiError(getErrorMessage(err, 'No pudimos restablecer tu contraseña. Intentá de nuevo.'))
    }
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Controller name="password" control={control} render={({ field, fieldState }) => (
          <PwField
            ...
            value={field.value}
            onChange={(v) => { field.onChange(v); if (apiError) setApiError(null) }}
            onBlur={field.onBlur}
      ...
      <p style={{ textAlign: 'center', margin: 0 }}>
        <Link
          ...
          style={{
            ...
            fontSize: 13, color: 'var(--ps-text-secondary)', textDecoration: 'none',
            fontWeight: 500,
          }}
        >
