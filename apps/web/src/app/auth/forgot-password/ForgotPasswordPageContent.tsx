      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, textAlign: 'center' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--ps-text-secondary)', margin: 0, lineHeight: 1.6, maxWidth: 320 }}>
            ...
            <span style={{ color: 'var(--ps-text-primary)', fontWeight: 600 }}>{email}</span>.
...
export function ForgotPasswordPageContent() {
  ...
  const onSubmit = async (data: ForgotValues) => {
    ...
    try {
      ...
      setApiError(getErrorMessage(err, 'No pudimos enviar el email. Intentá de nuevo.'))
    }
  }
    ...
    <AuthShell>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label htmlFor="forgot-email" style={{ fontSize: 13, fontWeight: 500, color: 'var(--ps-text-secondary)' }}>
            Email
          </label>
          <div style={{ position: 'relative' }}>
            <span style={{
              position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)',
              color: 'var(--ps-text-disabled)', pointerEvents: 'none',
