...
import { useCurrentOrganizationProfile, useUpdateProfile } from '@/lib/api/userApi'
...
function Field({ id, label, hint, error, children }: FieldProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label
        htmlFor={id}
        style={{ fontSize: 13, fontWeight: 500, color: 'var(--ps-text-primary)' }}
      >
        {label}
      </label>
      {children}
      {hint && !error && (
        <p style={{ margin: 0, fontSize: 12, color: 'var(--ps-text-secondary)' }}>{hint}</p>
      )}
      {error && (
        <p style={{ margin: 0, fontSize: 12, color: 'var(--ps-error)' }}>{error}</p>
      )}
    </div>
  )
}
...
export default function SettingsProfilePage() {
  ...
  }, [form, organizationQuery.data?.phone, user?.email, user?.first_name, user?.last_name])
  ...
  function reg(name: keyof ProfileFormValues) {
    ...
    return {
      ...rest,
      onFocus: focusInput,
      onBlur: (e: React.FocusEvent<HTMLInputElement>) => { blurInput(e); void rhfOnBlur(e) },
    }
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <form onSubmit={onSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 4 }}>
          ...
          >
            {isPending && <Loader2 size={14} strokeWidth={2} style={{ animation: 'spin 0.8s linear infinite' }} />}
