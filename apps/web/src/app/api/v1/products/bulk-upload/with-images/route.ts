/**
 * Custom API route for bulk-upload with-images to handle large ZIP files (60MB+).
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(
      `${apiUrl}/api/v1/products/bulk-upload/with-images`,
      {
        method: "POST",
        headers,
    ...
    console.error("[BulkUpload API Route] Error:", error);
