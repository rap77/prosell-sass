...
describe("Facebook Marketplace catalog", () => {
  ...
  it("uses unique accent-free snake_case keys within every field", () => {
    for (const field of FACEBOOK_FIELD_KEYS) {
      ...
      expect(
        keys.every((key) => /^[a-z0-9]+(?:_[a-z0-9]+)*$/.test(key)),
        field,
      ).toBe(true);
    }
  });
  ...
  it("keeps the source's inconsistent year IDs locale-specific", () => {
    ...
    expect(year1999).toMatchObject({
      es: "1999",
      en: "1999",
      facebookIds: { es: 29, en: 27 },
    });
  });

  it("returns all source matches for duplicate device IDs", () => {
    expect(
      getValuesByFacebookId("device", 60, "es").map((value) => value.es),
    ).toEqual(["OPPO F3 Plus", "OPPO F7"]);
  });

  it("looks up keys and country locales", () => {
    expect(findKeyBySpanishValue("vehicle_type", "Auto/camioneta")).toBe(
      "auto_camioneta",
    );
