...
type ColumnTheme = { dot: string; accent: string; overBg: string; overRing: string }

const COLUMN_THEME: Record<string, ColumnTheme> = {
  new:             { dot: 'var(--ps-cyan)',    accent: 'var(--ps-cyan)',    overBg: 'rgba(77,184,255,0.04)',   overRing: 'rgba(77,184,255,0.25)'   },
  contacted:       { dot: 'var(--ps-warning)', accent: 'var(--ps-warning)', overBg: 'rgba(245,166,35,0.04)',  overRing: 'rgba(245,166,35,0.25)'   },
  qualified:       { dot: 'var(--ps-success)', accent: 'var(--ps-success)', overBg: 'rgba(34,211,160,0.04)', overRing: 'rgba(34,211,160,0.25)'   },
  appointment_set: { dot: '#a78bfa',           accent: '#a78bfa',           overBg: 'rgba(139,92,246,0.04)', overRing: 'rgba(139,92,246,0.25)'   },
}

const FALLBACK_THEME: ColumnTheme = { dot: 'var(--ps-text-disabled)', accent: 'var(--ps-border-default)', overBg: 'var(--ps-hover-bg-xs)', overRing: 'var(--ps-border-default)' }
    ...
    >
      ...
      <div style={{ padding: '14px 14px 10px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {/* Status dot */}
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: theme.dot, flexShrink: 0, boxShadow: `0 0 6px ${theme.dot}` }} />
          {/* Label */}
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ps-text-primary)', flex: 1 }}>
        ...
        {/* Price totals */}
        {hasPrices && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0 8px', paddingLeft: 16 }}>
            {Object.entries(totalsByCurrency).map(([currency, cents]) => (
              <span key={currency} style={{ fontSize: 11, color: 'var(--ps-text-secondary)', fontWeight: 500 }}>
      ...
      >
        ...
        {leads.length === 0 && (
          <div style={{
            ...
            fontSize: 12, color: 'var(--ps-text-disabled)',
            border: `1px dashed var(--ps-border-subtle)`,
            borderRadius: 8, minHeight: 80, margin: '4px 0',
          }}>
