...
type StatusConfig = { label: string; bg: string; color: string; dot: string; pulse?: boolean }

const STATUS_CONFIG: Record<PublicationStatusType, StatusConfig> = {
  pending:    { label: 'Pendiente',    bg: 'var(--ps-warning-bg)', color: 'var(--ps-warning)', dot: 'var(--ps-warning)'          },
...
export function PublicationStatus({ status, errorCategory, blockedUntilConfirmed }: PublicationStatusProps) {
  ...
  const isBlocked = status === 'failed' && (errorCategory === 'blocking' || blockedUntilConfirmed === true)
    ...
    <>
      <span
        style={{
          ...
          fontWeight: 500,
          whiteSpace: 'nowrap',
          animation: config.pulse ? 'psStatusPulse 1.4s ease-in-out infinite' : undefined,
        }}
      >
