export function LandingProofStrip() {
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--ps-text-secondary)", margin: 0 }}>
      ...
      <div style={{ display: "flex", gap: 52, alignItems: "center", flexWrap: "wrap", justifyContent: "center" }}>
        {["MERIDIAN", "STACKFLOW", "NOVA GROUP", "AXION", "VELTRIX"].map((n) => (
          <span key={n} style={{ fontSize: 15, fontWeight: 700, letterSpacing: "0.18em", color: "var(--ps-text-secondary)" }}>{n}</span>
        ))}
      </div>
    </div>
  );
