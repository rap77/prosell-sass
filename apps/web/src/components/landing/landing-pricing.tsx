...
const CheckIcon = () => (
  <svg style={{ width: 11, height: 11 }} fill="none" stroke="currentColor" strokeWidth={2.75} viewBox="0 0 24 24">
  ...
  return (
    <section id="precios" style={{ position: "relative", padding: "100px 32px", borderTop: "1px solid var(--ps-border-subtle)", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1280, margin: "0 auto" }}>
        ...
        {/* Cards */}
        <div className="ps-pr-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1.06fr 1fr", gap: 24, alignItems: "start", marginBottom: 48 }}>
          {PRICING_PLANS.map((plan) => (
            <div key={plan.name} className={`ps-pr-card${plan.featured ? " ps-pr-card-featured" : ""}`}>
              {plan.badge && (
                <span style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", padding: "6px 14px", borderRadius: 100, background: "var(--ps-cyan)", color: "var(--ps-bg-base)", fontSize: 11, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", boxShadow: "0 0 18px rgba(77,184,255,0.40)", whiteSpace: "nowrap" }}>
                  {plan.badge}
                </span>
              )}
              ...
              <div style={{ fontSize: 40, fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1, marginBottom: 8 }}>
                {plan.price}
                {plan.priceUnit && <span style={{ fontSize: 16, fontWeight: 500, color: "var(--ps-text-secondary)" }}> {plan.priceUnit}</span>}
              </div>
