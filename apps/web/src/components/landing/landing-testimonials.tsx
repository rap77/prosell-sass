...
function TestimonialCard({ t }: { t: Testimonial }) {
  return (
    <article className="ps-tm-card">
      <div style={{ color: "var(--ps-warning)", fontSize: 13, letterSpacing: 1, marginBottom: 16 }}>★★★★★</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 38, height: 38, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "var(--ps-bg-base)", background: t.avBg }}>{t.av}</div>
        <div style={{ flex: 1 }}>
    ...
    <section style={{ position: "relative", padding: "100px 32px", borderTop: "1px solid var(--ps-border-subtle)", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20, marginBottom: 20 }}>
          {TESTIMONIALS_3.map((t) => <TestimonialCard key={t.name} t={t} />)}
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          {TESTIMONIALS_2.map((t) => <TestimonialCard key={t.name} t={t} />)}
        </div>
      </div>
    </section>
  );
