...
export function LeadList({ vendedorId, onLeadClick }: LeadListProps) {
  ...
  const { data: leads = [], isLoading, error, refetch } = useLeads(filters, limit, page * limit)
  ...
  if (error) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: 48, textAlign: 'center' }}>
        <p style={{ margin: 0, fontSize: 14, color: 'var(--ps-error)' }}>Error al cargar leads: {(error as Error).message}</p>
        <button
          type="button"
          onClick={() => refetch()}
          style={{ height: 36, padding: '0 16px', background: 'var(--ps-cyan)', color: 'var(--ps-bg-base)', border: 0, borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
        >
          Reintentar
        </button>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative', flex: '1 1 220px', maxWidth: 340 }}>
          ...
          <input
            ...
            onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--ps-cyan)'; e.currentTarget.style.boxShadow = '0 0 0 3px var(--ps-input-focus-shadow)' }}
            onBlur={(e)  => { e.currentTarget.style.borderColor = 'var(--ps-input-border)'; e.currentTarget.style.boxShadow = 'none' }}
          />
        </div>
        ...
        <button
          ...
          title="Actualizar"
        >
          <RefreshCw size={14} strokeWidth={2} style={{ animation: (isLoading || isManualRefetch) ? 'spin 0.8s linear infinite' : 'none' }} />
        </button>
      </div>
      ...
      >
        ...
        <div style={{
          ...
          <span />
          {COL_HEADERS.map((h) => (
            <span key={h} style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ps-text-disabled)' }}>
              {h}
            </span>
          ))}
        </div>
        ...
        {isLoading && leads.length === 0 && (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--ps-text-secondary)', fontSize: 13 }}>
            Cargando leads...
          </div>
        )}
        ...
        {!isLoading && leads.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: 48, textAlign: 'center' }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--ps-bg-elevated)', border: '1px solid var(--ps-border-default)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Users size={22} style={{ color: 'var(--ps-text-disabled)' }} strokeWidth={1.5} />
            </div>
        ...
        {leads.map((lead) => (
          ...
          >
            <LeadListItem
              ...
              onStatusUpdate={() => {/* handled by mutation */}}
            />
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8 }}>
        <button
          ...
          style={{
            ...
            color: page === 0 ? 'var(--ps-text-disabled)' : 'var(--ps-text-secondary)',
        ...
        <span style={{ fontSize: 12, color: 'var(--ps-text-secondary)', padding: '0 8px' }}>
          Página {page + 1}
        </span>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 8,
            fontSize: 13, fontWeight: 500,
            color: leads.length < limit ? 'var(--ps-text-disabled)' : 'var(--ps-text-secondary)',
