...
const OUTPUT_FILE = join(process.cwd(), '../../docs/tickets/baseline-results.json');
...
function printSummary(baseline) {
  ...
  console.log(`   /api/auth/state: ${baseline.metrics.api['/api/auth/state'].count}`);
  console.log(`   ℹ️  ${baseline.metrics.api['/api/auth/state'].note}`);

  console.log('\n📊 Lighthouse Metrics:');
  const lh = baseline.metrics.lighthouse;
  console.log(`   Performance Score: ${lh.performance}/100 ${lh.performance < 50 ? '❌' : lh.performance < 90 ? '⚠️' : '✅'}`);
