...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('dealer_id', sa.UUID(), sa.ForeignKey('user_dealers.id', ondelete='SET NULL'), nullable=True, index=True),
    ...
    op.create_foreign_key(
        'leads_vehicle_id_fkey', 'leads', 'vehicles', 
        ['vehicle_id'], ['id'], ondelete='SET NULL'
    )
    op.create_foreign_key(
        'appointments_vehicle_id_fkey', 'appointments', 'vehicles',
        ['vehicle_id'], ['id'], ondelete='SET NULL'
    )
