 ...
 * 7. Dealer calendar verification
 */

test.describe("MVP Complete Flow Validation", () => {
  const ADMIN_EMAIL = "admin@prosell.saas";
  ...
  test("MVP-001: Complete Admin Flow", async ({ page }) => {
    ...
    await test.step("Navigate to leads section", async () => {
      ...
      const leadUrls = ['/vendedor/leads', '/manager/team/leads', '/dealer/leads', '/leads'];
    ...
    await test.step("Navigate to appointments", async () => {
      ...
      const appointmentUrls = ['/dealer/appointments', '/calendar', '/appointments', '/vendedor/appointments'];
    ...
    await test.step("Check dealer calendar", async () => {
      ...
      if (hasCalendar) {
        console.log("✅ Dealer calendar component found");
        await page.screenshot({
          path: "mvp-e2e-validation/07-dealer-calendar.png",
          fullPage: true
        });
      } else {
