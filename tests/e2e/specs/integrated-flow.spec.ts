 ...
 */
testWithFixtures.describe(
  "Integrated Flow: Catalog → Lead → Appointment",
  () => {
    testWithFixtures(
      "Complete Happy Path: Catalog → Lead → Appointment",
      async ({ page, request }) => {
        // ============================================================================
        // SETUP
        // ============================================================================
        ...
        const authState = await page.request.get(
          "http://localhost:3000/api/v1/auth/state",
        );
        if (!authState.ok()) {
        ...
        const vehicleResponse = await request.get(
          `http://localhost:3000/api/v1/products/${vehicleId}`,
        );
        ...
        await expect(page.getByText("Chevrolet Equinox").first()).toBeVisible({
          timeout: 15000,
        });
        ...
        const leadId = await dataBuilder.createLead(vehicleId, {
          ...
          message:
            "I'm interested in this Chevrolet Equinox. Is it still available?",
        });
        ...
        const leadResponse = await request.get(
          `http://localhost:3000/api/v1/leads/${leadId}`,
        );
        ...
        expect(leadDetailData.lead.buyer_email).toMatch(
        ...
        console.log(
          `✅ Lead verified in API: ${leadDetailData.lead.buyer_name}`,
        );

        // 2.3 Verify lead appears in leads list
        const leadsListResponse = await request.get(
        ...
        const createdLead = leadsList.items.find(
        ...
        const updateStatusResponse = await request.put(
          `http://localhost:3000/api/v1/leads/${leadId}/status`,
          {
            data: {
        ...
        const qualifyLeadResponse = await request.put(
          `http://localhost:3000/api/v1/leads/${leadId}/status`,
          {
            data: {
        ...
        appointmentDate.setDate(
          appointmentDate.getDate() + 40 + (timestampSeed % 15),
        );
        while (
        ...
        const leadAfterAppointmentResponse = await request.get(
          `http://localhost:3000/api/v1/leads/${leadId}`,
        );
        expect(leadAfterAppointmentResponse.ok()).toBeTruthy();
        const leadAfterAppointmentDetail =
          await leadAfterAppointmentResponse.json();
        ...
        const createdAppointment = calendarData.items.find(
        ...
        const updateApptStatusResponse = await request.put(
          `http://localhost:3000/api/v1/appointments/${appointmentId}`,
          {
            data: {
              status: "completed",
        ...
        console.log(
          `✅ /branch/appointments page rendered (calendar UI loaded)`,
        );
        ...
        // Verify vehicle
        const verifyVehicleResponse = await request.get(
          `http://localhost:3000/api/v1/products/${vehicleId}`,
        );
        ...
        const verifyLeadResponse = await request.get(
          `http://localhost:3000/api/v1/leads/${leadId}`,
        );
        ...
        const verifyAppointmentResponse = await request.get(
          `http://localhost:3000/api/v1/appointments/${appointmentId}`,
        );
        ...
        console.log(
          `✅ VERIFICATION: Appointment ${appointmentId} → Lead ${leadId} → Vehicle ${vehicleId}`,
        );

        // Verify all entities in their respective lists
        const [allProductsList, allLeadsList, allAppointmentsList] =
          await Promise.all([
            request.get("http://localhost:3000/api/v1/products"),
            request.get("http://localhost:3000/api/v1/leads"),
            request.get(
              `http://localhost:3000/api/v1/appointments?dealer_id=${dealerId}`,
            ),
          ]);
        ...
        const listedProduct = productsData.products.find(
          (product: { id: string }) => product.id === vehicleId,
        );
        const listedLead = leadsData.items.find(
          (lead: { id: string }) => lead.id === leadId,
        );
        const listedAppointment = appointmentsData.items.find(
...
testWithFixtures.describe("Milestone C — Catalog detail page", () => {
  testWithFixtures(
    "UI: /catalog/{id} — product detail view",
    async ({ page, request }) => {
      await authenticateAsAdmin(page, request);
      const dataBuilder = new TestDataBuilder(page);
      const webBaseUrl = process.env.WEB_BASE_URL || "http://localhost:3000";

      const categoryId = await dataBuilder.createCategory(
      ...
      await page.waitForLoadState("networkidle");
      await expect(page.getByText("1HGCM82633A004321")).toBeVisible({
        timeout: 10000,
      });
...
test.describe("Milestone C — Publications page", () => {
  test("UI: /publications — publications list renders", async ({
    page,
    request,
  }) => {
    await authenticateAsAdmin(page, request);
    ...
    await page.waitForLoadState("networkidle");
    await expect(
      page.getByRole("button", { name: /nueva publicación/i }),
...
test.describe("Milestone C — Pipeline kanban", () => {
  test("UI: /pipeline — kanban board renders with columns", async ({
    page,
    request,
  }) => {
    await authenticateAsAdmin(page, request);
