...
export class VehiclesPage extends BasePage {
  ...
  readonly decodeVinButton = this.page.getByRole("button", {
    name: /decode|decodificar/i,
  });
  readonly decodedInfoSection = this.page.getByTestId(
    /decoded-info|vehicle-info/i,
  );
  ...
  readonly bodyTypeSelect = this.page.getByRole("combobox", {
    name: /tipo de carrocería|body type/i,
  });
  readonly drivetrainSelect = this.page.getByRole("combobox", {
    name: /tracción|drivetrain/i,
  });
  readonly transmissionSelect = this.page.getByRole("combobox", {
    name: /transmisión|transmission/i,
  });
  readonly fuelTypeSelect = this.page.getByRole("combobox", {
    name: /combustible|fuel type/i,
  });
  ...
  readonly exteriorColorSelect = this.page.getByRole("combobox", {
    name: /color exterior|exterior color/i,
  });
  readonly interiorColorSelect = this.page.getByRole("combobox", {
    name: /color interior|interior color/i,
  });

  // Buttons - FIXED: More specific selector for submit button
  readonly submitButton = this.page.getByRole("button", {
    name: /(create|save|crear)\s*(vehicle)?/i,
  });
  ...
  async selectCategory(categoryName: string): Promise<void> {
    ...
    const radixOption = this.page
      .locator(`[data-radix-select-item]`)
    ...
    try {
      ...
      console.log(
        `[SELECT CATEGORY] Radix option failed, trying role option...`,
      );
      // Try role option
      try {
        ...
        console.log(
          `[SELECT CATEGORY] Role option failed, trying text search...`,
        );
   ...
   */
  async getDecodedData(): Promise<{
    make?: string;
    model?: string;
    year?: string;
  }> {
    const make = await this.page
      .getByTestId("vehicle-make")
      .textContent()
      .catch(() => "");
    const model = await this.page
      .getByTestId("vehicle-model")
      .textContent()
      .catch(() => "");
    const year = await this.page
      .getByTestId("vehicle-year")
      .textContent()
      .catch(() => "");
  ...
  async verifyToastVisible(message: string | RegExp): Promise<void> {
    ...
    const isVisible =
      (await toastByAttr.isVisible().catch(() => false)) ||
