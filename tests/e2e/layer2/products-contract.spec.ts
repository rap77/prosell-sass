...
async function getVehiclesCategoryId(authenticatedRequest: any): Promise<string> {
  ...
  const vehiclesCategory = categories.find((cat: any) => cat.slug === 'vehicles');
...
test.describe('Layer 2: Product Lifecycle - Contract Validation', () => {
  ...
  test.describe('POST /api/v1/products - Create Product', () => {
    test('P-01: should create product with vehicle attributes', async ({ authenticatedRequest }) => {
    ...
    test('P-02: should reject product without title', async ({ authenticatedRequest }) => {
    ...
    test('P-03: should reject product with invalid price_cents', async ({ authenticatedRequest }) => {
    ...
    test('P-04: should reject product with invalid condition', async ({ authenticatedRequest }) => {
    ...
    test('P-05: should reject product without category_id', async ({ authenticatedRequest }) => {
      // Arrange: Missing required field
      const productData = {
        title: 'Test Product',
        price_cents: 1500000,
  ...
  test.describe('GET /api/v1/products - List Products', () => {
    test('P-06: should return paginated product list', async ({ authenticatedRequest }) => {
      // Act: Get product list
      const response = await authenticatedRequest.get('/api/v1/products?limit=10&offset=0');
    ...
    test('P-07: should filter products by status', async ({ authenticatedRequest }) => {
      // Act: Filter by status
      const response = await authenticatedRequest.get('/api/v1/products?status=draft');
    ...
    test('P-08: should filter products by category', async ({ authenticatedRequest }) => {
      // Act: Filter by category
      const response = await authenticatedRequest.get(`/api/v1/products?category_id=${categoryId}`);
    ...
    test('P-09: should filter products by condition', async ({ authenticatedRequest }) => {
      // Act: Filter by condition
      const response = await authenticatedRequest.get('/api/v1/products?condition=used');
    ...
    test('P-10: should filter products by price range', async ({ authenticatedRequest }) => {
      // Act: Filter by price range
      const response = await authenticatedRequest.get('/api/v1/products?min_price=1000000&max_price=2000000');
  ...
  test.describe('GET /api/v1/products/{id} - Get Product Details', () => {
    test('P-11: should return product details', async ({ authenticatedRequest }) => {
      ...
      const response = await authenticatedRequest.get(`/api/v1/products/${product.id}`);
    ...
    test('P-12: should return 404 for non-existent product', async ({ authenticatedRequest }) => {
      // Act: Get non-existent product
      const response = await authenticatedRequest.get('/api/v1/products/00000000-0000-0000-0000-000000000000');

      // Assert: Verify 404
      expect(response.status()).toBe(404);
    });

    test('P-13: should return 422 for invalid UUID format', async ({ authenticatedRequest }) => {
      // Act: Get product with invalid UUID
      const response = await authenticatedRequest.get('/api/v1/products/invalid-uuid-format');
  ...
  test.describe('PATCH /api/v1/products/{id} - Update Product', () => {
    test('P-14: should update product title', async ({ authenticatedRequest }) => {
      ...
      const response = await authenticatedRequest.patch(`/api/v1/products/${product.id}`, {
    ...
    test('P-15: should update product price', async ({ authenticatedRequest }) => {
      // Arrange: Create product
      const createResponse = await authenticatedRequest.post('/api/v1/products', {
        data: {
    ...
    test('P-16: should update product condition', async ({ authenticatedRequest }) => {
      // Arrange: Create product
      const createResponse = await authenticatedRequest.post('/api/v1/products', {
        data: {
    ...
    test('P-17: should update product attributes', async ({ authenticatedRequest }) => {
      // Arrange: Create product
      const createResponse = await authenticatedRequest.post('/api/v1/products', {
        data: {
  ...
  test.describe('Product Status Transitions', () => {
    test('P-18: should submit product for approval (draft → pending)', async ({ authenticatedRequest }) => {
      ...
      const response = await authenticatedRequest.post(`/api/v1/products/${product.id}/submit`);
    ...
    test('P-19: should approve product (pending → published)', async ({ authenticatedRequest }) => {
      ...
      // Act: Approve product
      const response = await authenticatedRequest.post(`/api/v1/products/${product.id}/approve`);
    ...
    test('P-20: should reject product (pending → rejected)', async ({ authenticatedRequest }) => {
      // Arrange: Create and submit product
      const createResponse = await authenticatedRequest.post('/api/v1/products', {
        data: {
      ...
      // Act: Reject product
      const response = await authenticatedRequest.post(`/api/v1/products/${product.id}/reject?reason=Test+rejection`);
    ...
    test('P-21: should pause product (published → paused)', async ({ authenticatedRequest }) => {
      ...
      // Act: Pause product
      const response = await authenticatedRequest.post(`/api/v1/products/${product.id}/pause`);
    ...
    test('P-22: should resume product (paused → published)', async ({ authenticatedRequest }) => {
      ...
      // Act: Resume product
      const response = await authenticatedRequest.post(`/api/v1/products/${product.id}/resume`);
    ...
    test('P-23: should mark product as sold (published → sold)', async ({ authenticatedRequest }) => {
      // Arrange: Create and publish product
      const createResponse = await authenticatedRequest.post('/api/v1/products', {
        data: {
      ...
      // Act: Mark as sold
      const response = await authenticatedRequest.post(`/api/v1/products/${product.id}/mark-sold`);
  ...
  test.describe('DELETE /api/v1/products/{id} - Delete Product', () => {
    test('P-24: should delete product', async ({ authenticatedRequest }) => {
      ...
      // Act: Delete product
      const response = await authenticatedRequest.delete(`/api/v1/products/${product.id}`);

      // Assert: Verify deletion
      expect(response.status()).toBe(204);

      // Verify product no longer exists
      const getResponse = await authenticatedRequest.get(`/api/v1/products/${product.id}`);
      expect(getResponse.status()).toBe(404);
    });

    test('P-25: should return 404 when deleting non-existent product', async ({ authenticatedRequest }) => {
      // Act: Delete non-existent product
      const response = await authenticatedRequest.delete('/api/v1/products/00000000-0000-0000-0000-000000000000');
